...
It's recommended to use this flag to disable it since it may waste performance in some scenarios, and you can use [Spark](https://spark.lucko.me/) as the profiling tool. Unless you want to profile your datapack functions.
...
It's recommended to use this flag to disable it since it may waste performance, especially on higher online counts. You can remove this flag only when you really need to use the debug feature.
...
Whether to enable the read-only mode for the Linear v2 region format. Under this mode, any world changes (including chunk, POI, entity, and player data) will **NOT be saved**.

This mode is intended for inspection, testing, migration, or emergency recovery.

Given that the Linear v2 has many design flaws and is dangerous to production, if you need to use the Linear region format, we recommend using the Buffered Linear instead.
