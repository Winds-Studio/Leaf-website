...
Es wird empfohlen diesen Parameter zu nutzen um ihn zu deaktivieren, da er in manchen Szenarien Leistung verschwenden kann, und du kannst [Spark](https://spark.lucko.me/) als Profiling-Tool verwenden. Es sei denn, du möchtest deine Datapack-Funktionen profilieren.
...
Ob die [Debug-Funktion](https://minecraft.wiki/w/Debug_property) von Minecraft vollständig deaktiviert werden soll.

Es wird empfohlen, dieses Flag zu nutzen, um sie zu deaktivieren, da sie Leistung verschwenden kann, insbesondere bei höheren Spielerzahlen. Diese Flag sollte nur entfernt werden, wenn die Debug-Funktionen wirklich benötigt werden.
...
Ob der schreibgeschützte Modus für das Linear v2 Region-Format aktiviert werden soll. In diesem Modus werden jegliche Änderungen an der Welt (einschließlich Chunk-, POI-, Entity- und Spielerdaten) **NICHT gespeichert**.

Dieser Modus ist für Inspektionen, Tests, Migrationen oder die Notfallwiederherstellung vorgesehen.

Da Linear v2 viele Designfehler aufweist und für den produktiven Betrieb riskant ist, empfehlen wir, stattdessen Buffered Linear zu verwenden, falls das Linear Region-Format benötigt wird.
