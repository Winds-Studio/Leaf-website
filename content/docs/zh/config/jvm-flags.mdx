...
是否完全禁用 MC 内置的 [调试功能](https://zh.minecraft.wiki/w/%E8%B0%83%E8%AF%95%E5%B7%A5%E5%85%B7)。

建议使用此 Flag 禁用调试功能，因为该功能可能会带来额外的性能开销，尤其是在在线人数较多时。

建议仅在确实需要使用调试功能时，才移除此启动参数。
...
是否为 Linear v2 区域格式启用只读模式。

在此模式下，任何世界数据改动（包括区块、POI、实体和玩家数据）都 **不会被保存**。

该模式主要用于数据检查、测试、迁移或紧急恢复等场景。

鉴于 Linear v2 存在较多设计缺陷，不建议在生产环境中使用。如果您需要使用 Linear 区域格式，推荐使用 Buffered Linear。
