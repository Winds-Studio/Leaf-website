...
const en: ConfigMessages<typeof config> = {
  async: {
    ...
    "async-entity-tracker": {
      ...
      "queue-size": {
        desc: `异步实体追踪任务队列的最大容量。<br>
                    如果设为 &leq; \`0\`，队列大小将会动态计算为 \`max-threads * 384\`。`,
      },
    },
    "async-locator": {
      enabled: {
    ...
    "async-target-finding": {
      "async-alert-other": {
        desc: `是否使用生物相互警告行为的异步执行。<br>
                    仅当上方的 \`enabled\` 为 \`true\` 时，此配置项才会生效。<br>
                    <br>
                    __⚡推荐值：\`true\`__`,
      },
      "async-search-block": {
        desc: `是否使用生物搜索方块目标的异步执行。<br>
                    仅当上方的 \`enabled\` 为 \`true\` 时，此配置项才会生效。<br>
                    <br>
                    __⚡推荐值：\`true\`__`,
      },
      "async-search-entity": {
        desc: `是否使用生物搜索实体目标的异步执行。<br>
                    仅当上方的 \`enabled\` 为 \`true\` 时，此配置项才会生效。<br>
                    <br>
                    __⚡推荐值：\`true\`__`,
      },
      enabled: {
        desc: `是否使用异步实体目标查找。<br>
                    <br>
                    生物（Mob）的目标搜索计算，例如寻找附近可攻击的实体或寻找附近可交互的方块，是非常消耗性能的，尤其是在生物数量很多或存在大量生物农场的情况下。<br>
                    通过将这些 AI 相关计算移动到其他线程，并在主线程中校验和执行结果，可以显著提升性能。<br>
                    <br>
                    __⚡推荐值：\`true\`__`,
      },
      "queue-size": {
        desc: `异步实体目标查找任务队列的最大容量。<br>
                    如果设为 &leq; \`0\`，队列大小将会动态计算为 \`max-threads * 384\`。`,
      },
    },
  ...
  misc: {
    ...
    "async-catcher": {
      enabled: {
        desc: `是否开启 Spigot 的线程安全检查。<br>
                    <br>
                    <div class="warning custom-block">
                    <p class="custom-block-title custom-block-title-default">警告</p>
                    不推荐禁用该配置项，否则可能导致严重的服务器不稳定问题。<br>
                    除非你非常清楚自己在干啥，否则请勿修改该配置项。
                    </div>`,
      },
    },
    cache: {
    ...
    "including-5s-in-get-tps": {
      desc: `是否在 \`Bukkit#getTPS\` 和 \`Server#getTPS\` API 方法的返回结果中包含 5 秒 TPS 数据。<br>
                ...
                <br>
                <div class="tip custom-block">
                <p class="custom-block-title custom-block-title-default">再深入一些？</p>
  ...
  performance: {
    ...
    "cache-eye-fluid-status": {
      desc: `是否缓存 \`Entity#isEyeInFluid\` 方法的结果，以提升性能并减少内存开销。<br>
                <br>
                __⚡推荐值：\`false\`__`,
    },
    ...
    "count-all-chunks-for-ticking": {
      desc: `是否跳过生物生成中判断已加载区块是否靠近玩家的检查。<br>
                <br>
                在大量玩家在线、已加载区块较多的场景下，该检查的开销可能较高。由于已加载区块通常本就位于玩家附近，跳过该检查通常是安全的，但在某些边缘情况下，生物生成量可能会略有增加。<br>
                <br>
                __⚡推荐值：\`true\`__`,
    },
    ...
    dab: {
      __desc__: `Dynamic Activation of Brain（简称 DAB）会在实体远离玩家时，降低其大脑 tick 计算的频率，以此优化实体 AI。若服务器中存在大量实体，这是一个值得采用的权衡优化和原版行为的优化方案。`,
      "activation-dist-mod": {
      ...
      "blacklisted-entities": {
        desc: `DAB 黑名单，名单内的生物将不受 DAB 的影响。<br>
                    ...
                    <br>
                    黑名单格式：\`[villager]\` 或 \`[villager, zombified_piglin]\`（可参见 [Paper 的 Javadoc](https://jd.papermc.io/paper/1.21.4/org/bukkit/entity/EntityType.html) 查看所有实体类型）。<br>
                    <br>
