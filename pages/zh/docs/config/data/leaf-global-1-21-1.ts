export default {
    ...
    "async": {
        "": {
            "default": "",
            "desc": "本节包含异步相关的特性, 旨在通过异步执行任务来减少主线程 (Server Thread) 的负载."
        },
        "async-entity-tracker": {
            ...
            "keepalive": {
                "default": 60,
                "desc": "空闲线程的超时时间, 超过该时间并且无任务的线程将被销毁.<br>" +
                    "(以 秒 为单位)"
            }
        },
    ...
    "performance": {
        "": {
            "default": "",
            "desc": "本节包含性能调优相关的配置, 旨在通过减少不必要地计算, 或采用更高效的逻辑 / 算法优化服务器."
        },
        ...
        "entity-timeouts": {
            "": {
                "default": "",
                "desc": "实体的最长存活时间 (又称 Entity TTL).<br>" +
                    "(以 tick 为单位)<br>" +
    ...
    "gameplay-mechanisms": {
        ...
        "player": {
            "disable-moved-wrongly-threshold": {
                "default": false,
                "desc": "是否禁用 Spigot 内置的 \"moved too quickly / wrongly\" 移动速度检查. (针对玩家和载具的移速).<br>" +
    ...
    "misc": {
        ...
        "sentry": {
            "": {
                "default": "",
                "desc": "[Sentry](https://sentry.io/welcome/) 是一个应用程序监控服务, 皆在更好地收集, 记录, 跟踪错误日志和相关信息, 协助运维人员更好的定位并修复问题.<br>" +
                    "<br>" +
        ...
        "region-format-settings": {
            "": {
                "default": "",
                "desc": "Linear 是一种区块文件格式, 使用 [ZSTD 压缩](https://facebook.github.io/zstd/) 代替 MC 原版的 ZLIB 压缩算法. 此格式可以节省约 ~50% 的磁盘空间.<br>" +
        ...
        "hidden-item-components": {
            "default": "[]",
            "desc": "是否不发送指定的物品组件信息给客户端. 这可能会破坏依赖该组件信息的资源包和客户端模组. 需要一个组件类型列表, 填写错误将会导致此功能无法正常工作.<br>" +
        ...
        "connection-message": {
            "": {
                "default": "",
                "desc": "玩家连接提示, 当玩家加入或退出服务器时广播发送给所有在线玩家.<br>" +
                    "需要使用 [MiniMessage](https://docs.advntr.dev/minimessage/format) 格式.<br>" +
