...
const config: ConfigRoot = {
        ...
        player: {
            ...
            "max-use-item-distance": {
                ...
                desc:
                    "玩家允许用物品进行交互的最远距离.<br>" +
                    "<br>" +
                    "一些 [无政府服](https://minecraftservers.org/type/anarchy) 或类似服务器可能允许玩家使用黑客端作弊。如果你希望这些玩家能够使用 基于发包的，与水晶相关的 功能 (例如 CEV Breaker、BedAura 等), 可能需要调整该配置项.<br>" +
    ...
    misc: {
        ...
        "including-5s-in-get-tps": {
            default: true,
            desc:
                "是否在 `Bukkit#getTPS` 和 `Server#getTPS` API 方法的结果中包含 5 秒 TPS 数据.<br>" +
                "<ul>" +
                ...
                '<details class="tip custom-block">' +
                '<summary class="custom-block-title custom-block-title-default">再深入一些？</summary>' +
                "如果你的插件基于 Gale API 或 Leaf API, 或者运行在 Leaf 上并使用反射来获取 TPS, 可以调用 `Bukkit#getTPSIncluding5SecondAverage` 方法来获取包含 5 秒 TPS 的 TPS 数组 (`5s, 1m, 5m, 15m`).<br>" +
                "同时，你也可以调用 `Bukkit#get5SecondTPSAverage` 来获取 5 秒 TPS 的平均值 (`double` 类型)." +
                "</details>"
        },
